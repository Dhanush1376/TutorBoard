...
import { sanitizeString } from '../../utils/logSanitizer.js';
...
function logUsage({ userId, provider, model, usage, responseTimeMs, taskType, success, errorType, isCustomKey, wasFallback, costCents }) {
  try {
    ...
    UsageLog.create({
      ...
      costEstimate: cost, isCustomKey: !!isCustomKey, wasFallback: !!wasFallback,
    }).catch(() => {});
  } catch (e) {
 ...
 *   7. Log every attempt
 */
export async function requestCompletion({ model, messages, temperature, maxTokens, tools, responseSchema, responseMimeType, userConfig, taskType }) {
  initClients();
  ...
  if (userConfig?.useCustomApi && userConfig?.decryptedKey && userConfig?.provider) {
    ...
    if (circuitBreaker.isAvailable(provider)) {
      ...
      try {
        ...
        const client = createProviderClient(provider, userConfig.decryptedKey, userConfig.baseUrl);
        const result = await executeWithRetry(client, provider, {
          model: userModel, messages, temperature, maxTokens, tools, response_format,
        }, 2, timeout.signal);
  ...
  try {
    ...
    logUsage({ userId, provider: 'openrouter', model: orModel, usage, responseTimeMs, taskType, success: true, isCustomKey: false, wasFallback, costCents: cost.costCents });
...
export async function requestCompletionRaced(params, primaryConfig, secondaryConfig) {
  ...
  const racePromises = racers.map(async (config, idx) => {
    ...
    try {
      const client = createProviderClient(config.provider, config.key, config.baseUrl);
