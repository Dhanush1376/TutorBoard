            ...
            {isGuest && (
              <div className={`px-4 py-3 border-b border-[var(--border-color)] ${usageWarning ? 'bg-amber-500/5' : 'bg-[var(--bg-primary)]'}`}>
                <div className="flex justify-between items-center mb-1.5">
                  <span className={`text-[10px] font-normal uppercase tracking-wider ${usageWarning ? 'text-amber-500' : 'text-[var(--text-secondary)]'}`}>
                    {usageWarning ? 'Trial Near Limit' : 'Trial Usage'}
                  </span>
                  <span className="text-[10px] font-normal text-[var(--text-primary)]">{usageCount}/{usageLimit} interactions</span>
                </div>
          ...
          {isGuest ? (
            <div className={`p-2.5 px-3 mb-2 rounded-xl border transition-colors ${usageWarning ? 'bg-amber-500/10 border-amber-500/30' : 'bg-[var(--bg-tertiary)] border-[var(--border-color)]/50'}`}>
               <div className="flex justify-between items-center mb-2">
                  <span className={`text-[9px] font-normal uppercase tracking-[0.2em] ${usageWarning ? 'text-amber-500' : 'text-amber-500/80'}`}>
                    {usageWarning ? 'Trial Near Limit' : 'Trial Usage'}
                  </span>
                  <span className="text-[9px] font-normal uppercase tracking-widest text-amber-500">
               ...
               <div className="mt-3 space-y-1">
                 <button 
                   onClick={() => { setIsOpen(false); onSettingsClick('appearance'); }}
                   className="w-full p-2 bg-[var(--bg-primary)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg text-[10px] font-normal flex items-center justify-center gap-2 transition-all"
                 >
                   <Settings size={12} />
                   Trial Settings
                 </button>
