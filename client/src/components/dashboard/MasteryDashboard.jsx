...
export default function MasteryDashboard() {
  ...
  const { learnerProfile, isMasteryOpen, setMasteryOpen } = useTutorStore();
          ...
          <AnimatePresence mode="wait">
            ...
            >
                    ...
                    <div className="lg:col-span-5 space-y-6">
                       ...
                       <div className="p-8 bg-[var(--bg-secondary)]/50 rounded-[2rem] border border-[var(--border-color)] space-y-4">
                          ...
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                <h4 className="text-xl font-medium">{data?.learningStyle || 'Visual & Logical'}</h4>
                                <p className="text-[10px] text-[var(--text-tertiary)]">Optimized for high-engagement simulations</p>
                             </div>
                          </div>
                       </div>
                    </div>
              ...
              {activeTab === 'history' && (
                <div className="space-y-8">
                   <header>
                    <h2 className="text-3xl font-light tracking-tight">System <span className="font-normal text-purple-400">Logs</span></h2>
                    <p className="text-[var(--text-secondary)] text-sm font-mono opacity-60">journalctl -u tutorboard-agent.service</p>
                  </header>
                  <div className="bg-black/40 rounded-2xl border border-[var(--border-color)] p-6 font-mono text-[11px] leading-relaxed overflow-hidden relative shadow-inner">
                    <div className="absolute top-0 left-0 w-full h-full bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] bg-[length:100%_2px,3px_100%] pointer-events-none z-10" />
                    <div className="space-y-2 max-h-[400px] overflow-y-auto no-scrollbar relative z-0">
                      {data?.confusionHistory?.slice().reverse().map((event, i) => (
                        <div key={i} className="flex gap-4 opacity-80 hover:opacity-100 hover:bg-white/5 transition-all p-1 rounded">
                          <span className="text-[var(--text-tertiary)] shrink-0">[{new Date().toLocaleTimeString()}]</span>
                          <span className="text-blue-400 shrink-0">INFO</span>
                          <span className="text-[var(--text-primary)]">
                            Detected interaction on <span className="text-yellow-500">"{event.topic}"</span>. 
                            Confusion index calculated at <span className={event.score > 0.6 ? 'text-red-400' : 'text-green-400'}>{(event.score * 100).toFixed(2)}%</span>.
                            Syncing with semantic vector store... <span className="text-green-500">DONE</span>
                          </span>
                        </div>
                      ))}
                      <div className="flex gap-2 text-blue-500 animate-pulse">
                        <span>&gt;</span>
