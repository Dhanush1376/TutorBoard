...
export default function MasteryDashboard() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-12 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      {/* Header */}
      <div className="flex flex-col space-y-2">
        <h1 className="text-4xl font-normal tracking-tight">Learner Intelligence</h1>
        <p className="text-[var(--text-secondary)] text-lg">Synthesizing your pedagogical journey across dimensions.</p>
      </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {topicEntries.length > 0 ? topicEntries.map(([topic, mastery]) => (
            ...
            <div className="col-span-full p-12 text-center bg-[var(--bg-tertiary)]/40 rounded-3xl border border-dashed border-[var(--border-color)] text-[var(--text-tertiary)]">
      ...
      <section className="space-y-6">
        ...
        <div className="bg-[var(--bg-secondary)] rounded-3xl border border-[var(--border-color)] overflow-hidden">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[var(--bg-tertiary)]/40 text-[10px] uppercase tracking-widest text-[var(--text-tertiary)]">
            ...
            <tbody>
              {doubtHistory?.slice(-5).map((doubt, i) => (
                <tr key={i} className="border-t border-[var(--border-color)] hover:bg-[var(--bg-tertiary)]/20 transition-colors cursor-pointer">
                  ...
                  <td className="px-6 py-4 text-right text-[var(--text-tertiary)] text-xs">
...
function StatCard({ label, value, color }) {
  ...
  return (
    <div className={`p-8 rounded-3xl border bg-gradient-to-br ${colors[color]} space-y-2 relative overflow-hidden`}>
      ...
      <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--text-primary)]/5 blur-3xl -mr-16 -mt-16 rounded-full" />
    </div>
  );
}
...
function TopicCard({ topic, mastery }) {
  return (
    <div className="p-6 bg-[var(--bg-secondary)] rounded-3xl border border-[var(--border-color)] hover:border-[var(--text-tertiary)]/30 transition-all group">
      <div className="flex justify-between items-start mb-6">
        <div className="p-3 bg-blue-500/10 rounded-2xl group-hover:bg-blue-500/20 transition-colors">
          <div className="w-6 h-6 bg-blue-500 rounded-lg blur-[2px] opacity-50" />
        </div>
        <span className="text-2xl font-normal text-[var(--text-primary)]/90">{Math.round(mastery * 100)}%</span>
      </div>
