...
export const StickyNoteShape = ({
  ...
  const { setCanvasLocked, setChatInputText, duplicateNote, toggleNotePin, bringToFront, pinnedNotes = [], setSelectedElements } = useTutorStore();
  ...
  const handleResizePointerDown = (e) => {
    ...
    const onMove = (me) => {
      if (onUpdate) {
        onUpdate({ 
          w: Math.max(0.1, (startW + (me.clientX - startX) / canvasScale) / CW), 
          h: Math.max(0.1, (startH + (me.clientY - startY) / canvasScale) / CH) 
        });
      }
    };
  ...
  return (
    ...
    >
      <rect 
        x={-width / 2} y={-height / 2} width={width} height={height} rx={2} 
        fill={col.bg} 
      ...
      <g transform={`rotate(-0.5, 0, ${-height / 2})`}><rect x={-24} y={-height / 2 - 10} width={48} height={22} rx={2} fill={col.tape} opacity={0.55} /></g>
      <foreignObject x={-width / 2 + 10} y={-height / 2 + 30} width={width - 20} height={height - 50}>
        <textarea 
          ...
          onPointerDown={(e) => {
            e.stopPropagation();
            setSelectedElements([layoutId]);
          }}
          style={{ width: "100%", height: "100%", background: "transparent", border: "none", outline: "none", resize: "none", padding: "2px 12px 12px", fontFamily: fontFamily || "'Caveat', cursive, sans-serif", fontSize: `${fontSize}px`, fontWeight: fontWeight || 600, overflow: "hidden", color: "#000" }} 
        />
      </foreignObject>
      <AnimatePresence>
        {(isSelected || isHovered) && (
          <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <g transform={`translate(${width / 2 - 12}, ${-height / 2 + 12})`} onClick={(e) => { e.stopPropagation(); onDelete?.(); }} style={{ cursor: "pointer" }}><circle r={10} fill={col.bg} stroke="#ef4444" strokeWidth={1.5} /><X size={12} x={-6} y={-6} stroke="#ef4444" /></g>
            <g transform={`translate(${-width / 2 + 35}, ${-height / 2 + 15})`} onClick={(e) => { e.stopPropagation(); setShowColorPicker(!showColorPicker); }} style={{ cursor: "pointer" }}><circle r={10} fill={col.bg} stroke="#000" strokeWidth={1.5} /><Palette size={12} x={-6} y={-6} stroke="#000" /></g>
            <g transform={`translate(${-width / 2 + 60}, ${-height / 2 + 15})`} onClick={(e) => { e.stopPropagation(); toggleNotePin(layoutId); }} style={{ cursor: "pointer" }}><circle r={10} fill={isPinned ? "#000" : col.bg} stroke="#000" strokeWidth={1.5} /><Pin size={12} x={-6} y={-6} stroke={isPinned ? "white" : "#000"} /></g>
          </motion.g>
        )}
      </AnimatePresence>
    </AW>
